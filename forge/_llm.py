...
# Per-call HTTP timeout. Was `None` (wait forever) which caused real-world hangs
# when OpenRouter's response stream broke mid-flight: the TCP socket stayed
# half-open and httpx blocked indefinitely. 10 minutes on read covers even
# slow gpt-5 reasoning runs; connect/write/pool stay tight so DNS or queueing
# issues fail fast. The retry loop in server.py catches httpx.TimeoutException
# (subclass of Exception) and retries up to 3 times per page.
...
def update_wiki_page(
    ...
    system = (
        ...
        "FRONTMATTER FORMAT — STRICT (do not relax these even when preserving the existing page):\n"
        "- `title` MUST remain a DOUBLE-QUOTED string. Never unquote it. Unquoted titles "
        "break the parser when they contain `:` (e.g. `Chomsky (1959): critiques`).\n"
        "- `summary` MUST be ONE double-quoted string on a single key — never a YAML list, "
        "never multi-line, never bullet points. If the existing summary contains the literal "
        "text `REPLACE WITH` or `<<...>>` markers, you MUST rewrite it as a real factual "
        "summary derived from the page body.\n"
        "- `tags` MUST be a YAML inline list of 3-5 lowercase keywords. If the existing tags "
        "field is the literal placeholder `[3-5 relevant lowercase keywords]` or contains "
        "`<<TAG...>>` markers, you MUST replace it with real tags derived from the page topic.\n"
        "- Only `aliases`, `sources`, and `tags` may be YAML lists.\n\n"
        "STRUCTURE — STRICT:\n"
        "- The See-Also section uses `## See Also` (exactly TWO hashes). If the existing "
        "page contains a stray `### See Also` (three hashes) anywhere, MERGE it into the "
        "single `## See Also` section and remove the duplicate."
    )
...
def synthesize_wiki_page(
    ...
    system = (
        ...
        "- `title` MUST be a DOUBLE-QUOTED string, ALWAYS. Unquoted titles break the parser "
        "on colons. Escape any literal `\"` inside as `\\\"`.\n"
        "- `summary` MUST be ONE double-quoted string on a single line — NEVER a YAML list, "
        "NEVER a multi-line block, NEVER bullet points. Inside the value use no double-quote "
        "characters.\n"
        "- `tags` MUST be a YAML inline list of 3-5 short lowercase keywords, hyphen-separated "
        "for multi-word terms.\n"
        "- Only `aliases`, `sources`, and `tags` may be YAML lists.\n\n"
        "PLACEHOLDER REPLACEMENT — CRITICAL:\n"
        "Any text inside `<<...>>` markers in the frontmatter is a PLACEHOLDER you MUST "
        "replace with the appropriate real value. The literal `<<...>>` markers MUST NEVER "
        "appear in your output.\n\n"
        "STRUCTURE — STRICT:\n"
        "- Use `## See Also` (exactly TWO hashes). Do NOT also create `### See Also` (three "
        "hashes) anywhere. Exactly ONE See-Also section per page, at heading level 2.\n"
        "- Output ONLY the complete markdown page. No preamble, no code fences, no postscript."
    )
    title_quoted = json.dumps(concept['title'])
    user = (
        f"Re-synthesize the wiki page for: {title_quoted}\n\n"
        ...
        f"{sources_block}\n"
        f"Copy this frontmatter block, REPLACING every `<<...>>` placeholder with the real "
        f"value derived from the sources:\n\n"
        f"---\n"
        ...
        f'summary: "<<WRITE_ONE_SENTENCE_SUMMARY_FROM_THE_SOURCES>>"\n'
        ...
        f"3. **`## See Also`** section (EXACTLY two hashes, never three): contains only the "
        f"single sentinel line `_To be linked after compilation._` — the cross-link pass "
        f"replaces it later.\n"
        f"4. **`## Sources`** section: a bullet per source, exactly:\n{sources_md_lines}\n"
        f"5. **`## Key Points`** section: 3-5 bullet takeaways drawn across sources.\n\n"
        f"{images_block}"
        f"Output ONLY the complete markdown page. No preamble, no code fences. Every "
        f"`<<...>>` placeholder must be replaced with a real value."
    )
    return _call(model, system, user, json_mode=False, max_output_tokens=32000)
...
def write_wiki_page(
    ...
    system = (
        ...
        "FRONTMATTER FORMAT — STRICT:\n"
        "- `title` MUST be a DOUBLE-QUOTED string, ALWAYS. Even when the title contains "
        "no special characters. Unquoted titles break the YAML parser on colons. "
        "Inside the quotes, escape any literal `\"` as `\\\"`.\n"
        "- `summary` MUST be ONE double-quoted string on a single line — NEVER a YAML list, "
        "NEVER a multi-line block, NEVER bullet points. Inside the value use no double-quote "
        "characters (use semicolons or rephrase). Wrong: `summary:\\n  - foo`. Right: "
        "`summary: \"foo; bar\"`.\n"
        "- `tags` MUST be a YAML inline list of 3-5 short lowercase keywords, hyphen-separated "
        "for multi-word terms: `tags: [machine-learning, attention, transformer]`. No spaces "
        "inside any individual tag.\n"
        "- Only `aliases`, `sources`, and `tags` may be YAML lists. Nothing else.\n\n"
        "PLACEHOLDER REPLACEMENT — CRITICAL:\n"
        "Any text inside `<<...>>` markers in the frontmatter I show you is a PLACEHOLDER "
        "that you MUST replace with the appropriate real value derived from the source. "
        "The literal `<<...>>` markers MUST NEVER appear in your final output. "
        "Outputting `<<...>>` or copying the placeholder text verbatim is a fatal error.\n\n"
        "STRUCTURE — STRICT:\n"
        "- Use `## See Also` (exactly TWO hashes) for the See-Also section. Do NOT also "
        "create `### See Also` (three hashes) anywhere in the body. There must be exactly "
        "ONE See-Also section per page, at heading level 2.\n"
        "- Output ONLY the complete markdown page (frontmatter + body). No preamble, no "
        "code fences (no ```markdown wrapper), no postscript."
    )
    title_quoted = json.dumps(concept['title'])
    user = (
        f"Write a comprehensive wiki page for the concept: {title_quoted}\n\n"
        f"Source document:\n---\n{source_text}\n---\n\n"
        f"Copy this frontmatter block, REPLACING every `<<...>>` placeholder with the real "
        f"value derived from the source:\n\n"
        f"---\n"
        f"title: {title_quoted}\n"
        f"slug: {concept['slug']}\n"
        f"category: concept\n"
        ...
        f"created: {date.today().isoformat()}\n"
        f"updated: {date.today().isoformat()}\n"
        f'summary: "<<WRITE_ONE_SENTENCE_SUMMARY_FROM_THE_SOURCE>>"\n'
        f"tags: [<<TAG1>>, <<TAG2>>, <<TAG3>>]\n"
        f"---\n\n"
        f"# {concept['title']}\n\n"
        f"Then write the page in this exact order:\n\n"
        ...
        f"{images_block}"
        f"Output ONLY the complete markdown page (frontmatter + body). No preamble, no code "
        f"fences, no postscript. Every `<<...>>` placeholder must be replaced with a real value."
    )
    return _call(model, system, user, json_mode=False, max_output_tokens=32000)
